...
# When your sample is smaller, will there be more or less standard error?
# More!

# What about if your original distribution sd was large?
# More!
...
mean.diff_sample <- f.mean - m.mean
...
mean.diff_shuffled <- f.mean_shuffled - m.mean_shuffled
...
means.diff_shuffled <- c()

for(i in 1:10000) {
  # (Re)Shuffle our labels
  p.shuffled <- penguins
  p.shuffled$sex <- sample(penguins$sex)
  ...
  # Calculate and save the difference in their means
  means.diff_shuffled[i] <- f.mean_shuffled - m.mean_shuffled
  
}


# Plot all the differences in means
hist(means.diff_shuffled)
abline(v = mean.diff_sample, col = "red", lwd = 2)


# What percent of our shuffled samples had differences in means more extreme than our difference in means?
sum(means.diff_shuffled < mean.diff_sample) / 10000 * 100

# Thought experiment: Control and treatment 
# Does our treatment condition help
# people be less depressed than our control condition?

# DV: reported depression
# IV: condition (control or treatment)

# Randomly shuffle the condition labels for your sample
# Do that 10,000 times to get a distribution of differences between the group
# Compare the difference between the two groups that we found in our experiment compared to the distribution of differences that we found
# And if what we found is sufficiently far away from our generated distribution
# (i.e., there are very few of our random samples that generated differences at
# least as extreme as the one that we found), then we can conlude that the

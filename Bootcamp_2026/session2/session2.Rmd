...
quack_responses_clean <- quack_responses_clean %>%
  ...
  mutate(CorrectAnswer = factor(CorrectAnswer, levels = c("Yes", "No")))
...
# let's remove the column with the most NAs and also remove the column Skills because they're very individualized responses

quack_responses_clean <- quack_responses_clean %>%
  select(-c(Languages, Skills))


# some other columns, like Lab, Languages, and Year have a few NAs here and there
...
Our data is looking pretty clean! One last thing: let's standardize the Pets column using Tidyverse.

```{r}

# in order to standardize responses in the Pets column, we're going to use gsub() to remove anything that is not a digit (i.e., all of the text in people's responses of how many pets they have)

quack_responses_clean <- quack_responses_clean %>%
  mutate(Pets = gsub("[^0-9]", "", Pets))

# you'll notice that this changed the data for someone who said that they have 2 dogs, 1 fish, 27 chickens to 2127. # of course, this is not what we want, so let's change that value to 30

quack_responses_clean$Pets[9] <- 30

# this also changed the value for someone who said "a few" to a blank. let's change it to "3" to approximate their original response. 

quack_responses_clean$Pets[14] <- 3
...
# great, now we have one column with everyone's experience levels (two rows per participant! now, let's summarize each participant's experience level. to do this, we're going to group_by() ID
...
```{r}
# for the sake of illustration, let's pretend our data on coding experience and our data on year in college are from two different dataframes. We have already created a new df called quack_coding_experience. Let's do the same for year in college. 


quack_year_college <- quack_responses_clean %>%
  select(c("ID", "Year")) 

# there are several ways we can combine these dfs, but let's use inner_join()	

combined_df <- inner_join(quack_year_college, quack_coding_experience, by = "ID")

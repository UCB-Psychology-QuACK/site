...
numbers <- c(1, 2, 3, 4, 5, 10000, 4000, 3) # after you run this, the variable 'numbers' should pop up in your global environment on the top right
...
names <-c("Sophie", "Nina", "Victoria", "Alyson", "Emily", "Sierra") 
...
# How is a factor different from a character vector? How do they show up in our global environment?
  ...
  groups_string<-c("Treatment1", "Treatment2", "Treatment3", "Treatment4", "Control")
  
  # making it a factor
  groups<-factor(c("Treatment1", "Treatment2", "Treatment3", "Treatment4", "Control"))
...
Once you are in the correct working directory, you can open up the files you want to see. When we are working in R, the most common file type we will be using is called a csv file, which stands for "comma separated value." To read in a csv file, you can use read.csv(). Make sure to save the csv file under some variable name (like quack_responses.) Don't forget to use quotation marks around the filename.

```{r}
quack_responses = read.csv("quack_response_data.csv")
```
...
getwd()
setwd("Folder 1/Foldy Fold/Mr. Folder/*The Best Part of waking up is/")
alien_smoothies = read.csv("alien_smoothies.csv")
```
...
pet_rock_survey = read.csv("Folder 1/Foldy Fold/Mrs. Folder/All you need is folders/pet_rock_survey.csv")
```
...
library(readr)
data <- read_csv("quack_response_data.csv")
...
answer1 <- data$`Is there a correct answer to this question?`[data$ID == 5]
...
answer1 <- data$LastQuestion[data$ID == 5]
...
data$SodaPop <- tolower(data$SodaPop)

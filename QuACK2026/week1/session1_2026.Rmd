...
names <-c("Alyson", "Nina", "Victoria", "Emily", "Sierra", "Becca") 
...
```{r}
quack_responses <- read.csv("quack_response_data_fall2026.csv")
```
...
Let's try viewing a dataset now that you know your way around the working directory!

```{r}

library(readr)
data <- read_csv("quack_responses_fa26.csv")
...
Sometimes, you'll want to index particular values in your dataframe. For example, you might want to know what respondent number 5 answered to the question "If yes, what have you used?" You can do that by referencing the column after a dollar sign (\$) and the row in square brackets ([]) like this:

```{r}

answer1 <- data$`If yes, what have you used?`[data$ID == 5]
# this line of code looks for the row where the column "ID" has a value of 5, and then for that row returns the value in the column "If yes, what have you used?"
...
answer1 <- data$Languages[data$ID == 5]

...
quack_responses_clean <- quack_responses %>%
  set_names(c("ID", "Timestamp", "Experience_R", "Experience_Other", "Languages", "Area", "BirthMonth", "Season", "FavTemp", "Pets", "Correct_Answer", "Hotdogs", "Bear_Rating"))
...
quack_responses_clean <- quack_responses_clean %>%
  mutate(FavTemp = as.numeric(str_remove_all(FavTemp, "ºF| F")))

# another way to standardize responses is using Tidyverse
# in the Pets column, let's use a combination of mutate(), case_when(), and str_detect() to standardize responses

quack_responses_clean <- quack_responses_clean %>%
  mutate(Pets = case_when(str_detect(Pets, "Over ") ~ "",
                            TRUE ~ Pets))

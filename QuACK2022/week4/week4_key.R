...
penguins_analyses_dropNAfirst <- penguins %>% 
...
## ORDER MATTERS!  Dropping NA first results in a different df!
...
# - Set age eligibility
# ^^ could do this either with filter (e.g., filter out anyone who doesn't meet
# an age requirement) or you could add an "eligibility" column
...
# new column such that if penguins weight > median then "heavy"
...
# case_when body_mass_g > median penguin then "heavy"
...
penguins_class <- penguins %>% 
  ...
  mutate(heavyLight = case_when(body_mass_g >= median(body_mass_g, na.rm = TRUE) ~ "heavy",
                                body_mass_g < median(body_mass_g, na.rm = TRUE) ~ "light",
                                TRUE ~ NA_character_),
         species_updated = case_when(island == "Torgersen" & species == "Adelie" ~ "Newdelie",
                                     TRUE ~ species))
